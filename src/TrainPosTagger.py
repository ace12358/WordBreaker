    ...
    for epoch in range(n_epoch):
        ...
        for line in open(train_file):
            ...
            for target in range(len(x)):
                pos = x[target].split('/')[1]
                pred, loss = forward_one(x, target, pos, hidden, prev_c, True)
                accum_loss += loss
            batch_count += 1
...
def forward_one(x, target, label, hidden, prev_c, train_flag):
    ...
    correct = get_onehot(pos2id[label])
...
def epoch_test(char2id, model, epoch):
    ...
    for line in open(test_file):
        ...
        for target in range(len(x)):
            pos = x[target].split('/')[1]
            dist, acc = forward_one(x, target, pos, hidden, prev_c , train_flag=True)
            dists.append(dist)
        with open(result_file, 'a') as test:
            test.write("{0}\n".format((merge_wp(x, dists))))
    os.system('cat {0} |python mecab_format.py > mecab_format_result'\
                                            .format(result_file))
    os.system('cat {0} |python mecab_format.py > mecab_format_test_result'\
                                            .format(test_file))

    os.system('/usr/local/libexec/mecab/mecab-system-eval mecab_format_result mecab_format_test_result > temp')
